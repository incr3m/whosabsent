...
if(isset($_POST['idno'])){
	...
	if(isModeIn([MODE_CREATE])=='true'){
		$q = "INSERT INTO TeacherSchedule (teacheridno,sectionsubjectidno) 
		values ($teacheridno,$sectionsubjectidno)";	
	}
	else if(isModeIn([MODE_UPDATE])=='true'){
		$q = "update TeacherSchedule
		 set 
		 sectionsubjectidno = '$sectionsubjectidno'
		 where idno = $id ";	
	}
	else if(isModeIn([MODE_DELETE])=='true'){
		$q = "delete from TeacherSchedule where idno = $id";	
	}
  ...
  if ($result = $myCon->query("SELECT e.idno,b.code as sectioncode,c.code as subjectcode,DAYNAME(CONCAT('1970-09-2', d.dayofweek-1)) as day,DATE_FORMAT(d.starttime,'%h:%i %p') as starttime,DATE_FORMAT(d.endtime,'%h:%i %p') as endtime FROM Teacher a,section b,subjectunit c, sectionsubject d, teacherschedule e where a.idno = e.teacheridno and d.idno = e.sectionsubjectidno and d.subjectidno = c.idno and d.sectionidno = b.idno and a.idno = $objId ")) {
